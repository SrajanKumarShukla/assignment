...
### java.lang.AssertionError: assertion failed: bad position: [132:131]
...
class RateRepository @Inject()(db: Database) {
  ...
  def searchBuyingRates(
    ...
    db.withConnection { conn =>
      val baseQuery = new StringBuilder("SELECT * FROM rates WHERE rate_type = 'Buying'")

      // Collect conditions dynamically
      val conditions = List(
        mode.map(v => s"mode = '$v'"),
        orgCountry.map(v => s"origin_country_code = '$v'"),
        destCountry.map(v => s"destination_country_code = '$v'"),
        orgPort.map(v => s"origin_port_code = '$v'"),
        destPort.map(v => s"destination_port_code = '$v'")
      ).flatten
